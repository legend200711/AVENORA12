...
async function _createLiveFeedPost(creatorData) {
  ...
  try {
    const postRef = await addDoc(collection(_db, 'posts'), {
      ...
      liveRoomId:    _roomId,
      isLive:        true,
      title:         creatorData.title        || 'Avenora LIVE',
...
async function _notifyFollowersLive(creatorData) {
  ...
  try {
    ...
    const notif = {
      ...
      roomId:     _roomId,
      roomTitle:  creatorData.title       || 'Avenora LIVE',
      title:      '🔴 ' + (creatorData.hostName || 'Someone') + ' is Live',
      body:       `${creatorData.hostName || 'Someone'} is live: ${creatorData.title || 'Avenora LIVE'}`,
